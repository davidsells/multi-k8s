...
docker build -t davidsells/multi-server -t davidsells/multi-server:$SHA  -f ./server/Dockerfile ./server
docker build -t davidsells/multi-worker -t davidsells/multi-worker:$SHA  -f ./worker/Dockerfile ./worker
